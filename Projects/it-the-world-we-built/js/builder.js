...
window.WorldBuilder = (function () {
  ...
  function animate() {
    ...
    for (const key in components) {
      ...
      ctx.fillStyle = node.active ? '#FFFFFF' : '#F4EFFB';
      ctx.strokeStyle = node.active ? '#7C3AED' : 'rgba(139, 92, 246, 0.25)';
      ctx.lineWidth = node.active ? 3 : 1.5;
      ctx.shadowColor = node.active ? 'rgba(124, 58, 237, 0.25)' : 'transparent';
      ctx.shadowBlur = node.active ? 16 : 0;
      ctx.fill();
      ctx.stroke();
      ctx.shadowBlur = 0;
      ...
      // Label below
      ctx.font = node.active ? '600 12px Plus Jakarta Sans, sans-serif' : '500 11px Plus Jakarta Sans, sans-serif';
      ctx.fillStyle = node.active ? '#1B1035' : '#8B7FA8';
